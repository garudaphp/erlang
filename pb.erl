...
element_at([], _) -> "Elemt index is greater then list size";
element_at([H | _], 1) -> H;
...
encode(List) -> encode(reverse(List), [], {}).
encode([H | T], Acc, {}) -> encode(T, Acc, {H, 1});
encode([], Acc, Pack) -> [Pack | Acc];
encode([H | T], Acc, {H, N}) -> encode(T, Acc, {H, N + 1});
encode([H | T], Acc, Pack) -> encode(T, [Pack | Acc], {H, 1}).

encode_modified(List) -> encode_modified(List, [], {}).
encode_modified([H | T], Acc, {}) -> encode_modified(T, Acc, {H, 1});
encode_modified([], Acc, Pack) -> [Pack | Acc];
encode_modified([H | T], Acc, {H, N}) -> encode_modified(T, Acc, {H, N + 1});
encode_modified([H | T], Acc, {X, N}) when N == 1 -> encode_modified(T, [X | Acc], {H, 1});
encode_modified([H | T], Acc, Pack) -> encode_modified(T, [Pack | Acc], {H, 1}).
